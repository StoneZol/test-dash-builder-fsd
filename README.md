...
| Виджет | Назначение |
| ------ | ---------- |
| **Table** | Таблица стран (мультивыбор + detail queries) |
| **Statistics** | Метрика населения по выбранной стране |
| **Chart** | Население по регионам (CSS-бары) |
| **News card** | Карточка-брифинг по стране (не лента новостей) |

Источник данных — REST Countries. Наборы опций в селектах зависят от окружения (см. ниже).

**Почему News card — брифинг, а не «новости».** У выбранного API нет новостной ленты: только справочник стран. Чтобы закрыть слот «новостная карточка» из ТЗ без второго источника данных, виджет показывает карточку-брифинг из country detail (название, регион, столица, население, флаг) в формате news-card UI. Отдельный news-feed API сознательно не тащили — один домен проще для архитектуры и демо.
